...
import {
    LEGAL_MCP_TOKEN_REGISTRY_FILE_MAX_BYTES,
...
} from '../src/services/legal-research/mcp-config';
...
function createPrivateRegistry(body = registryJson()): {
    ...
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), 'legal-mcp-registry-')));
    roots.push(root);
...
describe('법률 MCP 보호 파일 registry', () => {
    ...
    it('relative/non-canonical path, 약한 parent/file mode, symlink를 거부한다', () => {
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1('clients.json'),
            LegalMcpTokenRegistryFileError
        );
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(weakParent.filePath),
            LegalMcpTokenRegistryFileError
        );
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(weakFile.filePath),
            LegalMcpTokenRegistryFileError
        );
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(symlinkPath),
            LegalMcpTokenRegistryFileError
        );
    });
    ...
    it('missing path와 directory target을 regular registry로 취급하지 않는다', () => {
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(directoryPath),
            LegalMcpTokenRegistryFileError
        );
    });
    ...
    it('file mode 000과 0640을 모두 거부한다', () => {
        for (const mode of [0o000, 0o640]) {
            ...
            assert.throws(
                () => readLegalMcpTokenRegistryFileV1(candidate.filePath),
                LegalMcpTokenRegistryFileError
            );
        }
    });
    ...
    it('지원 플랫폼에서는 FIFO를 non-regular target으로 즉시 거부한다', (context) => {
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(fifoPath),
            LegalMcpTokenRegistryFileError
        );
    });
    ...
    it('1..16KiB byte 제한과 strict registry schema를 fail-closed 한다', () => {
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(empty.filePath),
            LegalMcpTokenRegistryFileError
        );

        const oversized = createPrivateRegistry(
            'x'.repeat(LEGAL_MCP_TOKEN_REGISTRY_FILE_MAX_BYTES + 1)
        );
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(oversized.filePath),
            LegalMcpTokenRegistryFileError
        );
        ...
        assert.throws(
            () => readLegalMcpTokenRegistryFileV1(extraKey.filePath),
            LegalMcpTokenRegistryFileError
        );
    });
    ...
    it('오류 문자열에 registry path, JSON 원문 또는 digest를 포함하지 않는다', () => {
        ...
        assert.throws(
            ...
            (error: unknown) => {
                ...
                return error instanceof LegalMcpTokenRegistryFileError;
            }
        );
    });
    ...
    it('startup 설정 검증과 health용 source/count가 path나 clientId 없이 계산된다', () => {
        const { filePath } = createPrivateRegistry();
        const state = getLegalMcpConfigurationStateV1({
        ...
        const runtimeState = getLegalMcpConfigurationStateV1({
            lawApiOc: 'registered-account',
            tokenSha256: '',
            tokenRegistryJson: '',
            tokenRegistryFile: filePath,
    ...
    it('FILE/JSON/legacy 중 둘 이상이면 startup에서 비활성화한다', () => {
        ...
        for (const conflicting of [
            ...
            const state = getLegalMcpConfigurationStateV1(conflicting);
    ...
    it('stable malformed fingerprint는 본문을 한 번만 읽고 동시·후속 요청을 모두 거부한다', async () => {
        ...
        assert.equal(
            concurrent.every((result) =>
                ...
                && result.reason instanceof LegalMcpTokenRegistryFileError
            ),
            true
        );
        assert.equal(reloads, 1);
    ...
    it('runtime health 검사는 auth provider를 공유하고 invalid/fail-recover를 반영한다', async () => {
        ...
        assert.equal(
            (await getLegalMcpRuntimeConfigurationStateV1(input, provider))
                .configured,
            true
        );
        replaceRegistryAtomically(filePath, '{');
        const invalid = await getLegalMcpRuntimeConfigurationStateV1(
            input,
            provider
        );
        ...
        const recovered = await getLegalMcpRuntimeConfigurationStateV1(
            input,
            provider
        );
    ...
    it('startup invalid로 /mcp가 미mount된 경우 파일만 복구해도 health를 true로 바꾸지 않는다', async () => {
        const { filePath } = createPrivateRegistry('{');
        const input = {
            lawApiOc: 'registered-account',
            tokenSha256: '',
        ...
        const startup = getLegalMcpConfigurationStateV1(input);
        assert.equal(startup.configured, false);
        replaceRegistryAtomically(filePath, registryJson('late-recovery'));

        const health = await getLegalMcpRuntimeConfigurationStateV1(
            input,
            undefined,
            startup
        );
