...
export const GIS_MCP_TOKEN_REGISTRY_FILE_MAX_BYTES = 16 * 1024;

export class GisMcpTokenRegistryFileError extends Error {
    constructor(message = 'GIS_MCP_TOKEN_REGISTRY_FILE을 안전하게 읽을 수 없습니다.') {
        super(message);
        this.name = 'GisMcpTokenRegistryFileError';
    }
}
...
interface GisMcpTokenRegistryFileSnapshot {
...
export interface GisMcpTokenRegistryFileProviderInstrumentationV1 {
...
export interface GisMcpTokenRegistryFileProviderV1 {
    getStartupRegistryV1(): GisMcpTokenRegistryV1;
    readRegistryV1(): Promise<GisMcpTokenRegistryV1>;
    isForPathV1(filePath: string): boolean;
}

function fileError(): never {
    throw new GisMcpTokenRegistryFileError();
}

function invalidSnapshotError(fingerprint: SecureFileFingerprint): never {
    const error = new GisMcpTokenRegistryFileError();
...
function isProtectedRegistryFile(info: BigIntStats, uid: bigint): boolean {
    ...
    return info.isFile()
        ...
        && info.size >= 1n
        && info.size <= BigInt(GIS_MCP_TOKEN_REGISTRY_FILE_MAX_BYTES);
}
...
function parseRegistryBody(body: Buffer, bytesRead: number): GisMcpTokenRegistryV1 {
    const json = body.subarray(0, bytesRead).toString('utf8');
    return parseGisMcpTokenRegistryJson(json);
}

function readSnapshotSync(filePath: string): GisMcpTokenRegistryFileSnapshot {
    ...
    try {
        ...
        body = Buffer.alloc(GIS_MCP_TOKEN_REGISTRY_FILE_MAX_BYTES + 1);
        let bytesRead = 0;
        ...
        if (
            bytesRead < 1
            || bytesRead > GIS_MCP_TOKEN_REGISTRY_FILE_MAX_BYTES
        ...
        if (error instanceof GisMcpTokenRegistryFileError) throw error;
        throw new GisMcpTokenRegistryFileError();
    } finally {
        body?.fill(0);
...
async function inspectSecureMetadataAsync(
    filePath: string,
    instrumentation?: GisMcpTokenRegistryFileProviderInstrumentationV1
...
async function readSnapshotAsync(
    filePath: string,
    instrumentation?: GisMcpTokenRegistryFileProviderInstrumentationV1
): Promise<GisMcpTokenRegistryFileSnapshot> {
    const candidate = canonicalCandidate(filePath);
    ...
    try {
        ...
        let registry: GisMcpTokenRegistryV1;
        try {
            registry = parseRegistryBody(body, bytesRead);
        } catch {
...
export function readGisMcpTokenRegistryFileV1(
    filePath: string
): GisMcpTokenRegistryV1 {
    return readSnapshotSync(filePath).registry;
}

export async function readGisMcpTokenRegistryFileAsyncV1(
    filePath: string
): Promise<GisMcpTokenRegistryV1> {
    return (await readSnapshotAsync(filePath)).registry;
}

class GisMcpTokenRegistryFileProvider
implements GisMcpTokenRegistryFileProviderV1 {
    private snapshot: GisMcpTokenRegistryFileSnapshot | null;
    ...
    constructor(
        ...
        private readonly instrumentation:
            GisMcpTokenRegistryFileProviderInstrumentationV1 = {}
    ) {
        try {
    ...
    async readRegistryV1(): Promise<GisMcpTokenRegistryV1> {
        ...
        try {
            ...
            try {
                ...
                if (error instanceof GisMcpTokenRegistryFileError) {
...
export function createGisMcpTokenRegistryFileProviderV1(
    filePath: string,
    instrumentation: GisMcpTokenRegistryFileProviderInstrumentationV1 = {}
): GisMcpTokenRegistryFileProviderV1 {
    return new GisMcpTokenRegistryFileProvider(filePath, instrumentation);
}
