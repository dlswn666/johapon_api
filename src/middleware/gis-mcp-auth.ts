...
} from './gis-mcp-token-registry-file';
...
export interface GisMcpTokenVerifierOptions {
    /** 생략하면 process.env.GIS_MCP_TOKEN_SHA256을 읽는다. */
    tokenSha256?: string;
    /** 생략하면 process.env.GIS_MCP_TOKEN_REGISTRY_JSON을 읽는다. */
    tokenRegistryJson?: string;
    /** 생략하면 process.env.GIS_MCP_TOKEN_REGISTRY_FILE을 읽는다. */
    tokenRegistryFile?: string;
    /** index에서 auth와 health가 공유하는 file snapshot provider. */
    tokenRegistryFileProvider?: GisMcpTokenRegistryFileProviderV1;
...
interface ConfiguredGisMcpTokenSource {
    readTokens(): Promise<ConfiguredGisMcpToken[]>;
}
...
function configurationError(error: unknown): never {
    if (
        error instanceof GisMcpTokenRegistryConfigurationError
        || error instanceof GisMcpTokenRegistryFileError
    ) {
...
function createConfiguredTokenSource(
...
): ConfiguredGisMcpTokenSource {
    const legacyValue =
        options.tokenSha256 ?? process.env.GIS_MCP_TOKEN_SHA256;
    const registryValue =
    ...
    if (
        ...
        throw new GisMcpAuthConfigurationError(
            'GIS_MCP_TOKEN_REGISTRY_FILE, GIS_MCP_TOKEN_REGISTRY_JSON, GIS_MCP_TOKEN_SHA256 중 정확히 하나를 설정해야 합니다.'
        );
    }
    ...
    if (hasRegistry) {
        try {
            const tokens = configuredTokensFromRegistry(
                parseGisMcpTokenRegistryJson(registryValue)
            );
...
export function createGisMcpTokenVerifier(
    ...
    return {
        async verifyAccessToken(token: string): Promise<AuthInfo> {
            let configuredTokens: ConfiguredGisMcpToken[];
            try {
            ...
            return {
                ...
                extra: {
                    // rate-limit 주체를 bearer 세대에 묶어 rotation 뒤 구 세대를 분리한다.
