...
5. 건축물 분류만 확정되지 않았지만 공통 scope gate의 다른 차단 사유가 없고, 한 PNU의 활성 `property_unit`이 정확히 1건이며 그 물건과 연결 `building_unit` 후보 전체에 동·층·호·등기 외부 ID가 전혀 없는 parcel-level singleton이면 `ladfrlList` 후보를 허용한다. 이 경로도 필지 범위와 토지 소유를 SYSTEM_ADMIN이 현재 hash에 대해 확인해야 하며, unit identity가 하나라도 있거나 다물건이면 적용하지 않는다.
6. 위 5번의 parcel-level singleton 조건을 충족하지 않는 연립주택, 아파트, 다중주택, 비주거, 복합용도, 대장 코드 혼재는 이번 자동 적용 범위에서 제외하고 `REVIEW_REQUIRED`로 끝낸다.
7. 외부 API 오류·불완전 페이지·매칭 중복은 0건 응답으로 취급하지 않는다.
8. 수동값도 다음 명시적 관리자 동기화에서 유효한 자동값으로 덮어쓸 수 있다. 단, 자동 수집 근거가 완전해야 하고 값이 달라지는 MANUAL 덮어쓰기는 별도 확인을 요구한다.
...
2026-07-24 미아7 POSITIVE 표본에서 `clsSeCode=0`, `ldaQotaRate` 공백,
`buldDongNm/buldFloorNm/buldHoNm/buldRoomNm`이 모두 `0000`인 placeholder가
유효 7호와 함께 정확히 1행 관찰됐다. 이 exact shape만 비적용 관찰로 분리하고
`RATIO_PARSE_FAILED` review issue를 남기되 유효 행 적용을 막지 않는다. 하나라도
shape가 다르거나 placeholder가 2행 이상이면 기존처럼 전체 blocking한다.
...
4a. source↔EXPOS 층·호 exact와 root가 이미 입증됐지만 legacy `building_unit.floor`
    등이 비어 있는 경우, 후보의 값이 있는 모든 동·층·호가 source와 exact 일치하고
    `ho`가 존재하며 후보 전체의 `building_id`가 정확히 하나로 입증된 범위에서 후보가
    정확히 1건일 때만 기존 building-unit 링크를 사용한다. 누락 DB 컬럼을 채우거나
    추정하지 않으며 building 범위가 0/2개 이상이거나 후보가 0/2건 이상이면 무변경한다.
