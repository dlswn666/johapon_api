...
marker_path="${app_root}/.legal-mcp-file-registry-v1"
registry_dir="${app_root}/.legal-mcp-secrets"
registry_file="${registry_dir}/clients.json"
container_registry_dir="/run/secrets/tonghari-legal-mcp"
container_registry_file="${container_registry_dir}/clients.json"
container_name="alimtalk-proxy"
unknown_marker="${app_root}/.legal-mcp-registry-commit-unknown"
receipts_dir="${app_root}/.legal-mcp-registry-receipts"
run_parent="${app_root}/.legal-mcp-registry-operations"
...
name_prefix="legal-mcp-registry-${REGISTRY_RUN_ID}-${REGISTRY_RUN_ATTEMPT}"
...
load_operation_marker() {
  ...
  loaded_outcome="${marker_lines[9]#outcome=}"
  [[ "${marker_lines[1]}" == "runKey=${loaded_run_key}" \
    ...
    && "${marker_lines[9]}" == "outcome=${loaded_outcome}" \
...
assert_receipt_ledger_safe() (
  ...
  local receipt_post_state receipt_outcome
  ...
  for receipt in "${receipt_entries[@]}"; do
    ...
    [[ "${#receipt_lines[@]}" -eq 10 && "${receipt_lines[0]}" == "version=3" ]] \
      || return 1
    ...
    receipt_outcome="${receipt_lines[9]#outcome=}"
    [[ "${receipt_lines[1]}" == "runKey=${receipt_run_key}" \
      ...
      && "${receipt_lines[9]}" == "outcome=${receipt_outcome}" \
...
operation_marker_payload() {
  ...
  printf \
    ...
    "${marker_pre_state}" "${marker_post_count}" "${marker_post_state}" "${outcome}"
}
...
count_registry_updaters() {
  ...
  while IFS= read -r name; do
    [[ -n "${name}" ]] || continue
    if [[ "${name}" =~ ^legal-mcp-registry-[0-9]+-[0-9]+-updater$ ]]; then
      count="$((count + 1))"
    fi
  done <<< "${names}"
  printf '%s\n' "${count}"
...
if [[ "${mutation}" -eq 1 ]]; then
  [[ ! -e "${unknown_marker}" && ! -L "${unknown_marker}" ]] \
    || fail "COMMIT_STATE_UNKNOWN: an earlier mutation requires guarded recovery."
  assert_receipt_ledger_safe \
...
readonly_cli() {
  ...
  docker run --rm --name "${exact_name}" \
    ...
    "${running_image}" node /app/dist/cli/legal-mcp-registry.js "$@" \
...
create_mutation_container() {
  ...
  created_id="$(docker create -i --name "${updater_name}" \
    ...
    "${running_image}" node /app/dist/cli/legal-mcp-registry.js "${action}" \
      ...
      process.stdin.on("end", () => {
        try {
          ...
          const ok=h.status === "ok" && h.gitSha === process.env.EXPECTED_GIT_SHA
            ...
            && String(f.legalMcpRegisteredClientCount) === process.env.EXPECTED_CLIENT_COUNT
            && String(f.legalMcpRegisteredTokenCount) === process.env.EXPECTED_CLIENT_COUNT;
          process.exit(ok ? 0 : 1);
        } catch { process.exit(1); }
      });' >/dev/null
}
...
inspect_recovery_evidence() {
  ...
  if [[ ( -e "${stale_receipt}" || -L "${stale_receipt}" ) \
    ...
    || -e "${stale_receipt_temp}" || -L "${stale_receipt_temp}" ]]; then
    [[ "${loaded_outcome}" =~ ^(verified|known-precommit)$ ]] \
      || fail "Recover receipt evidence requires a terminal marked outcome."
    [[ -d "${receipts_dir}" && ! -L "${receipts_dir}" \
      && "$(realpath -e -- "${receipts_dir}")" == "${receipts_dir}" \
      && "$(stat -c '%u:%a' "${receipts_dir}")" == "$(id -u):700" ]] \
    ...
    if [[ -e "${stale_receipt}" || -L "${stale_receipt}" ]]; then
      [[ -f "${stale_receipt}" && ! -L "${stale_receipt}" \
        ...
        && "$(sha256sum -- "${stale_receipt}" | awk '{print $1}')" == "${recovery_unknown_sha256}" ]] \
        || fail "Recover receipt does not exactly match the pending marker."
      ...
      [[ -f "${stale_receipt_temp}" && ! -L "${stale_receipt_temp}" \
        ...
        && "$(sha256sum -- "${stale_receipt_temp}" | awk '{print $1}')" == "${recovery_unknown_sha256}" ]] \
        || fail "Recover receipt temporary file does not exactly match the pending marker."
...
remove_recovery_evidence() {
  ...
  if [[ "${recovery_receipt_mode}" == "receipt" ]]; then
    [[ -f "${recovery_receipt_path}" && ! -L "${recovery_receipt_path}" \
      ...
      && "$(sha256sum -- "${recovery_receipt_path}" | awk '{print $1}')" == "${recovery_unknown_sha256}" \
      && ! -e "${recovery_receipt_temp}" && ! -L "${recovery_receipt_temp}" ]] \
      || fail "Recovery receipt evidence changed after inspection."
    sync -d "${recovery_receipt_path}" \
      || fail "Recovery receipt could not be flushed."
    sync -f "${receipts_dir}" \
      || fail "Recovery receipt directory could not be flushed."
    ...
    [[ -f "${recovery_receipt_temp}" && ! -L "${recovery_receipt_temp}" \
      ...
      && "$(sha256sum -- "${recovery_receipt_temp}" | awk '{print $1}')" == "${recovery_unknown_sha256}" \
      && ! -e "${recovery_receipt_path}" && ! -L "${recovery_receipt_path}" ]] \
      || fail "Recovery receipt temporary evidence changed after inspection."
